...
def P_Ap1(k,P,f):
	...
	Z1_high=lambda r : -224.+(384-768*f/5.)*r**2+(-1152./5+2304./35*f)*r**4+(1152-256*f)/35.*r**6 + (128/35.-256/385.*f)*r**8
	Z1_low=lambda r: 32./5-768*f/5. + (-1152+2304*f)/35./r**2 +(-128-256*f)/35./r**4 +(-384-256*f)/385./r**6 +(-384/1001.-768.*f/5005.)/r**8
...
def P_Ap3(k,P,f):
	...
	Z3_high=lambda r : -448.*f+ (3072.*f-768*f**2)/5.*r**2+(-13824.*f+2304.*f**2)/35.*r**4+(2048.*f-256.*f**2)/35.*r**6 + (512/77.*f-256/385.*f**2)*r**8
	Z3_low=lambda r: -704*f/5.-768*f**2/5. + 2304.*f**2 /35./r**2 +(-512*f-256*f**2)/35./r**4 +(-1024*f-256*f**2)/385./r**6 +(-4608.*f-768.*f**2)/5005./r**8
...
def P_Ap5(k,P,f):
	...
	Z5_high=lambda r : f**2*(-224.+(1152/5.)*r**2 - 1152./7.*r**4+ 128./5.*r**6 + 1152./385.*r**8)
	Z5_low=lambda r: f**2*(-736./5. + 1152/35./r**2 -384./35./r**4 -128./77./r**6 -384./715./r**8)
